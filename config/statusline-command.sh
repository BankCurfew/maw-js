...
# Per-session file so rrr hook reads the correct oracle's data
_sid=$(echo "$input" | jq -r '.session_id // ""' 2>/dev/null)
[ -n "$_sid" ] && echo "$input" > "$TDIR/statusline-${_sid}.json" 2>/dev/null
# Per-cwd file so hooks can find current oracle by working directory
...
# Single line — Claude Code statusline only renders 1 line in pane
echo "📡 ${pct}% ${used_k}k/${max_k}k • ${dur} • ${model} • ${sid}${git}"
